...
class PurchaseHistorySync {
  ...
  Future<PurchaseHistoryPullResult> pullFromSavedDesktop() async {
    ...
    if (healthBody is! Map) throw StateError('Desktop health response invalid');
    ...
    final response = await _client
        .get(Uri.parse('$normalized/api/v1/purchases'), headers: headers)
    ...
    await db.transaction((txn) async {
      for (final raw in body['items'] as List) {
        ...
        final mapped = await mappedLocalId(txn, 'purchase', remoteId);
        final localId = mapped ?? remoteId;
        ...
        final row = <String, Object?>{
          'id': localId,
          'purchase_no': remote['purchase_no']?.toString().trim().isNotEmpty == true
          ...
          'discount_cents': (remote['discount_cents'] as num?)?.toInt() ?? 0,
          'tax_cents': (remote['tax_cents'] as num?)?.toInt() ?? 0,
          'delivery_fee_cents':
              (remote['delivery_fee_cents'] as num?)?.toInt() ?? 0,
          'other_fee_cents': (remote['other_fee_cents'] as num?)?.toInt() ?? 0,
          'source': remote['source']?.toString() ?? 'desktop',
          'draft_id': remote['draft_id']?.toString().trim().isEmpty == true
              ? null
              : remote['draft_id']?.toString(),
          // Desktop file paths are intentionally never copied to Android.
          'image_path': existing.isEmpty
              ? ''
              : existing.first['image_path']?.toString() ?? '',
